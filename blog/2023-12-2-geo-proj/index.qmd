...
The analysis hinges on three key data sources: VIIRS night light data, OpenStreetMap data detailing roads and buildings, and American Community Survey (ACS) socio-economic data. We employed R, utilizing libraries such as `stars`, `sf`, and `terra`, to manipulate and analyze these datasets. Our methodology involved a step-by-step approach, beginning with the aggregation of night light intensity data to assess blackout areas. We then integrated this with road and building data to estimate the number of homes affected and concluded with an examination of socio-economic factors, offering insights into the recovery dynamics of different communities.

Night lights data: "Level-1 and Atmosphere Archive & Distribution System Distributed Active Archive Center - LAADS DAAC." NASA. Accessed December 5, 2023. https://ladsweb.modaps.eosdis.nasa.gov/.

Roads data: "Planet OSM." Index of /. Accessed December 5, 2023. https://planet.openstreetmap.org/.
...
At this stage of the analysis, it is time to get down to the main analysis and determine the potential difference in income between impacted versus unimpacted tracts. To conduct this analysis, we will createa a map of median income by census tract and a box-plot of the income in impacted versus unimpacted tracts.
...
map <- ggplot(data = acs_data) +
  ...
  labs(title = "Median Income by Census Tract in Houston",
       subtitle = "Tracts impacted by blackouts are outlined in red",
       fill = "Median Income") +
...
boxplot <- ggplot(acs_data, aes(x = impact_status, y = B19013e1, fill = impact_status)) +
  ...
  labs(title = "Quartile Plot of Median Income in Houston's Census Tracts",

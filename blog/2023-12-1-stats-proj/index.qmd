...
Next, let's perform a linear regression on the economic freedom index and methane emissions.
...
```{r}
find_similar_countries_w_methane <- function(data, target_country, top_n = 25) {
  # Filter the dataset for the year 2020
  data_2020 <- filter(data, Year == 2020)

  # Select relevant columns
  relevant_data <- data_2020 %>%
    dplyr::select(country_name, economic_freedom_summary_index, x1_size_of_government, x2_legal_system_property_rights_with_gender_adjustment, x3_sound_money, x4_freedom_to_trade_internationally, x5_regulation) %>%
    na.omit()
  ...
  emissions_2020 <- filter(methane_emissions, Year == 2020)
  ...
  # Return the top similar countries
  head(similar_countries_with_emissions, top_n)
}
```

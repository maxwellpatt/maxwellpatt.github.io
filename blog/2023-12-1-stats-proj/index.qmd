...
```{r}
# Calculate the year-over-year change for each country
renewable_data <- freedom_esg %>%
  arrange(country_name, Year) %>%
  group_by(country_name) %>%
  mutate(Yearly_Change = Value - lag(Value))

# Filter the data for the years 2000 to 2020
renewable_data_filtered <- renewable_data %>%
  filter(Year >= 2000, Year <= 2020)

# Calculate the average change per quartile
average_change_per_quartile <- renewable_data_filtered %>%
  group_by(quartile) %>%
  summarize(Average_Change = mean(Yearly_Change, na.rm = TRUE))

# View the results
average_change_per_quartile
```
